...
def _generate_cloudflare(prompt: str, cfg: dict) -> bytes:
    """Cloudflare Workers AI. Free tier: 10,000 neurons/day, no card required.

    One image a day sits so far inside that allowance it will never be close.
    """
    import requests
    ...
    resp = requests.post(
        ...
        json={"prompt": prompt, "steps": cfg.get("image_steps", 8)},
        timeout=180,
    )
    ...
    # FLUX returns JSON with a base64 image; the SDXL family returns raw bytes.
    ...
    if "application/json" in ctype:
        ...
        if not b64:
            raise RuntimeError(f"No image in response: {str(payload)[:600]}")
        return base64.b64decode(b64)

    return resp.content


def _generate_gemini(prompt: str, cfg: dict) -> bytes:
    """Kept for the day this becomes worth a dollar a month. Needs billing enabled."""
...
def generate_image(prompt: str, cfg: dict, out_path: Path) -> None:
    ...
    if provider == "cloudflare":
        data = _generate_cloudflare(prompt, cfg)
    elif provider == "gemini":
        data = _generate_gemini(prompt, cfg)
    else:
